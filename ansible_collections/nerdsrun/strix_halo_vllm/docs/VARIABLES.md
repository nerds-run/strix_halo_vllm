...
| `lemonade_slot_profile` | string | `"single"` | Which entry of `lemonade_slot_profiles` to apply. Applied on demand by `lemonade_slots.yml`, never by a routine converge — it reloads the model |
...
| `lemonade_cache_ram_mib` | int | `8192` | `--cache-ram`, the in-memory prompt-cache pool. **The default is badly undersized here** — entries measure 1.3-2.6 GiB, so the pool holds three to five and evicts on every request. See the cache-cliff section in `PERFORMANCE.md` |
