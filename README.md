...
> **Note:** The `nemotron` and `super` profiles require llama.cpp build **≥8351** (fixes [ggml-org/llama.cpp#20570](https://github.com/ggml-org/llama.cpp/issues/20570) — mamba-base.cpp assertion crash). Both are hybrid Mamba-Transformer architectures. The `super` profile runs at the **full native 1,048,576-token context** (~76 GB total — model + KV) on a 128 GB Strix Halo. The hybrid LatentMoE design keeps KV growth nearly flat with context (Mamba-2 layers have constant state, only attention layers scale), so 1M fits with ~48 GB to spare for cache-reuse and other workloads. Tool calling rides on `--jinja` + the GGUF's chat template; the vLLM-only `--tool-call-parser qwen3_coder` / `--reasoning-parser super_v3` flags are NOT supported by `llama-server` and will crash it.
>
